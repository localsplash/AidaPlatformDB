...
access checks in [issue #8](https://github.com/localsplash/AidaPlatformDB/issues/8).
...
```yaml
include:
  - ../AidaPlatformDB/echo/compose.yaml
```
...
(see `.env.example`): `ECHO_NETWORK` and `ECHO_DB_HOST` to reach MySQL,
...
```bash
docker compose -f ../AidaPlatformDB/echo/compose.yaml --env-file /path/to/env up
```
